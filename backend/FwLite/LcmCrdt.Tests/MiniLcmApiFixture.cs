...
public class MiniLcmApiFixture : IAsyncLifetime, IAsyncDisposable
{
    ...
    public async Task InitializeAsync(string projectName, Guid? projectId = null)
    {
        ...
        if (_configureServices is not null)
        {
            ...
            // Hook config like HarmonyConfig gets baked into the EF model, which EF caches process-wide
            // (first fixture wins). Give this fixture its own EF service provider so its config applies.
            serviceCollection.AddDbContextFactory<LcmCrdtDbContext>(builder => builder.EnableServiceProviderCaching(false));
        }
